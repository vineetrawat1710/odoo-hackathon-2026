...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="h-16 border-b border-slate-200/80 bg-white/80 backdrop-blur-md flex items-center justify-between px-8 fixed top-0 right-0 left-64 z-10">
      {/* Title */}
      <div>
      ...
      <div className="flex items-center gap-5">
        ...
        <button
          ...
          className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all duration-200 focus:outline-none disabled:opacity-50"
        ...
        <div className="relative">
          <button
            ...
            className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all duration-200 relative focus:outline-none"
          >
            <Bell className="h-4.5 w-4.5" />
            <span className="absolute top-1.5 right-1.5 h-1.5 w-1.5 rounded-full bg-indigo-600"></span>
          </button>
          ...
          {showNotifications && (
            <>
              ...
              />
              <div className="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-xl shadow-crystal-hover z-40 py-2 animate-fade-in">
                <div className="px-4 py-2 border-b border-slate-100 flex items-center justify-between">
                  <span className="text-xs font-semibold text-slate-800">Operational Alerts</span>
                  <span className="text-[10px] text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded font-medium">2 New</span>
                </div>
                <div className="divide-y divide-slate-50">
                  {mockAlerts.map(alert => (
                    <div key={alert.id} className="px-4 py-3 hover:bg-slate-50/60 transition-colors">
                ...
                <div className="px-4 py-2 border-t border-slate-100 text-center">
                  <button 
                    ...
                    className="text-[10px] text-indigo-600 hover:text-indigo-700 font-semibold focus:outline-none"
                  >
                    Clear all alerts
                  </button>
                </div>
