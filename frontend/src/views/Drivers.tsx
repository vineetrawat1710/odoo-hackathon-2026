...
export const Drivers: React.FC = () => {
  ...
  const getExpiryBadge = (dateString: string) => {
    if (!dateString) return <span className="font-semibold text-slate-700">Unknown</span>;
    ...
    if (diffDays < 0) {
      return <span className="px-1.5 py-0.5 text-[9px] uppercase tracking-wider rounded-sm bg-rose-100 text-rose-700 font-bold">Expired</span>;
    } else if (diffDays <= 30) {
      return <span className="px-1.5 py-0.5 text-[9px] uppercase tracking-wider rounded-sm bg-amber-100 text-amber-700 font-bold">Expiring Soon</span>;
    }
    return <span className="px-1.5 py-0.5 text-[9px] uppercase tracking-wider rounded-sm bg-emerald-100 text-emerald-700 font-bold">Valid</span>;
  };
            ...
            <Card key={d.id} className="hover:shadow-md transition-shadow cursor-pointer">
              <CardContent className="p-5">
                <div className="flex items-start gap-4">
                  <div className="h-11 w-11 rounded-full bg-indigo-100 flex items-center justify-center font-bold text-indigo-800 text-sm flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-100">
                      ...
                      <div className="flex items-center gap-1 text-[11px] text-slate-500">
                        ...
                        Exp: {getExpiryBadge(d.license_expiry_date)}
                      </div>
                    </div>
