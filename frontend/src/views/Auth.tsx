...
export const Auth: React.FC = () => {
  ...
  const inputClass = 'w-full px-4 py-2.5 text-sm border border-slate-300 rounded-[10px] bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all placeholder:text-slate-400';
  const labelClass = 'block text-xs font-semibold text-slate-700 mb-1.5';

  return (
    <div className="min-h-screen bg-[#fafbfc] dotted-bg flex items-center justify-center px-4 py-12 font-sans">
      <div className="w-full max-w-md">

        {/* Brand */}
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2.5 mb-4 cursor-pointer" onClick={() => navigate('/')}>
            <div className="bg-gradient-to-br from-indigo-600 to-violet-600 text-white p-2 rounded-xl shadow-crystal-btn">
              <Compass className="h-6 w-6" />
            </div>
            <div className="text-left">
              <span className="text-xl font-bold text-slate-900 tracking-tight">TransitOps</span>
              <span className="text-[10px] text-indigo-500 font-semibold block uppercase tracking-wider -mt-1">Fleet Management Portal</span>
            </div>
          </div>
        </div>

        {/* Card */}
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-crystal overflow-hidden">
          ...
          <form className="px-8 py-6 space-y-5" onSubmit={view === 'login' ? handleLogin : view === 'signup' ? handleSignup : handleForgot}>
            ...
            {view === 'login' && (
              <div className="flex justify-end">
                <button type="button" className="text-xs text-indigo-600 hover:text-indigo-700 font-medium focus:outline-none" onClick={() => switchView('forgot')}>
                  Forgot password?
                </button>
              </div>
            )}
          ...
          <div className="px-8 py-5 bg-slate-50/50 border-t border-slate-100 text-center text-xs text-slate-500">
            {view === 'login' && (
              <span>
                ...
                <button className="text-indigo-600 hover:text-indigo-700 font-semibold focus:outline-none" onClick={() => switchView('signup')}>
                  Register now
                </button>
              </span>
            )}
            {view === 'signup' && (
              <span>
                Already registered?{' '}
                <button className="text-indigo-600 hover:text-indigo-700 font-semibold focus:outline-none" onClick={() => switchView('login')}>
                  Sign in instead
                </button>
              </span>
            )}
            {view === 'forgot' && (
              <button className="flex items-center gap-1.5 mx-auto text-indigo-600 hover:text-indigo-700 font-semibold focus:outline-none" onClick={() => switchView('login')}>
